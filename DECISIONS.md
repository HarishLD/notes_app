...
## `UnauthorizedError`'s message became a constructor parameter, defaulting to session-expiry copy; `authenticateUser` passes its own
**Why:** `UnauthorizedError` is thrown from three places with two genuinely different meanings — an invalid/expired/missing session (`jwt.ts`, `session.ts`) versus a failed signin (`authenticateUser`) — and the known bug was that a failed signin rendered "Not authenticated," which is correct for the first case and confusing on a login form. Following the pattern `NotFoundError`/`ConflictError` already use (a configurable message), the default became "Your session has expired. Please sign in again." (what the session-related call sites actually mean and the only one of the three that's realistically user-visible — `requireUser()`'s throw in `app/notes/page.tsx` is caught and redirected, never rendered as text, but the same class propagating from a notes/tags API 401 after a session expires mid-session *is* shown, in `note-card.tsx`/`create-note-form.tsx`/`tag-select.tsx`'s existing error banners). `authenticateUser` passes `"Invalid email or password."` explicitly at its one throw site — no branching added anywhere else, per the task's instruction to fix this by throwing with the right message rather than by teaching the UI to guess which case it's in.
**Critical invariant preserved:** `authenticateUser` still has exactly one `throw` covering both the unknown-email and wrong-password branches (`if (!user || !isValid)`) — there was never a second branch to diverge, so the message change couldn't accidentally make the two cases distinguishable. The existing test asserting identical status and body for both branches still passes unchanged.
**Alternative:** A dedicated `InvalidCredentialsError` subclass would separate the two concerns more explicitly, but adds a class whose entire difference from `UnauthorizedError` is its default string — CLAUDE.md's error hierarchy has no other single-purpose message-only subclasses, and one wasn't needed once the message became configurable.

## Zod's default validation messages replaced with plain-language ones; `ValidationError`'s generic message changed too
**Why:** Zod v4's defaults are technically accurate but not something to show a user — confirmed by actually running the schemas (`signupSchema.safeParse({ password: "short1" })` returns `"Too small: expected string to have >=8 characters"`, and a bad email returns `"Invalid email address"`). Added a custom message to every `.min()`/`.max()`/`z.email()` call a user can actually trigger through a form (signup/signin email and password, note title/body, tag name). `ValidationError`'s own top-level message ("Invalid input") became "Please check your input and try again." — shown as the banner text in every form alongside (or, for tag creation, instead of — `tag-select.tsx` has no per-field error slot for the tag-name input) the specific field messages, so it had to stay generic enough to make sense without assuming per-field highlighting exists everywhere it's used.
**Not changed:** `noteQuerySchema`'s field messages (`sort`, `q`, `tags`) — confirmed these are never actually shown to a user. `SortSelect` only ever sends one of two fixed option values (never a value zod would reject), and a malformed `q`/`tags` reaching the page is caught by `noteQuerySchema.safeParse` and silently falls back to defaults rather than throwing (an existing, documented decision — see "A malformed `searchParams` value falls back to defaults" above) — `GET /api/notes`'s own validation of the same schema is a real API-consumer-facing 400, not a browser-UI-facing one, since the notes page calls `listNotes` directly rather than fetching its own API. `updateNoteSchema`'s `.refine()` message ("At least one field is required") is similarly never reachable through the UI — `NoteForm` always submits both `title` and `body` — so it was left alone rather than inventing copy for an input state nothing in the app produces.
**Verified no test broke:** grepped every `.test.ts` file for `.error` and `message` assertions before making any of these changes — none exist; every existing validation test only asserts `result.success`, confirming message text was free to change. All 113 tests still pass unchanged after the edit.

## `NotFoundError` and `ConflictError` given explicit, call-site-specific messages instead of a shared default
**Why:** `NotFoundError`'s only prior message was its constructor default, "Not found," used identically for a missing/foreign note (`lib/notes/service.ts`, three call sites) and, inside `setNoteTags`, for two different failure modes that shouldn't share a name — the note not existing versus one of the submitted tag ids not existing. The note-related sites now all read "This note doesn't exist or has already been deleted." (deliberately identical whether the note never existed or belongs to someone else — CLAUDE.md's 404-not-403 existence-leak rule applies exactly as much to wording as it does to status code); the tag-ownership check in `setNoteTags` gets its own "One or more selected tags don't exist." — distinguishing "your note" from "your tag selection" leaks nothing, since both checks are already scoped to the caller's own `userId`. `ConflictError`'s two call sites (`registerUser`, `createTag`) got more specific, actionable copy: "An account with this email already exists. Try signing in instead." and "You already have a tag with this name."
**Alternative:** Leaving `NotFoundError`'s constructor default ("Not found") to cover all five call sites is less code, but produces the exact kind of message that "restates the error type rather than saying what happened" the task called out — passing an explicit message at each throw site, per the task's own instruction, was the more direct fix.

## Generic 500 body given an actionable next step
**Why:** `toErrorResponse`'s unknown-failure branch is the one 500 response the app can ever emit without leaking Prisma detail (CLAUDE.md §4.5) — the message has to stay fully generic. Changed "Something went wrong" to "Something went wrong on our end. Please try again in a moment." — same amount of real information (none, deliberately), but explicitly places the fault on the server ("our end," not something the user did) and gives the one honest next step available (retry). Client-side fallback strings for the same "non-2xx response that doesn't match the expected shape" edge case (`login-form.tsx`, `signup-form.tsx`, `note-form.tsx`, `note-card.tsx`, `tag-select.tsx`) were given the equivalent treatment for consistency, since they're the same category of message shown in the same category of situation.
**Alternative:** None — this is copy-only, no status code or `code` field changed, and no test asserts this string (confirmed by grep before editing).
